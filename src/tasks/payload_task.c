...
void vPayloadTask(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    if (xTaskNotifyWait(0x00, 0xFFFFFFFF, &notify_value, pdMS_TO_TICKS(PAYLOAD_PERIOD_MS)) ==
        pdTRUE)
    {
        ...
        if (camera_init())
        {
          if (camera_capture(5000))
          {
            ...
            if (len > 0 && len < 200000) /* Safety limit 200KB */
            {
              ...
              {
                char filename[32];
                snprintf(filename, sizeof(filename), "/IMAGES/img_%lu.jpg", (unsigned long)counter);
    ...
    if (fmm_get_mode() == FM_PAYLOAD)
    {
      ...
      /* 1Hz: Radiation */
      if ((counter % RADIATION_PERIOD_DIV) == 0)
      {
    ...
    counter++;
    vTaskDelayUntil(&xLastWakeTime, pdMS_TO_TICKS(PAYLOAD_PERIOD_MS));
  }
}
