...
static void cmd_help(const char *cmd)
{
  (void)cmd;
  uart1_acquire_lock();
  uart1_write_unsafe("[CMD] ── System ──\r\n");
  uart1_write_unsafe("[CMD] REBOOT              reboot OBC\r\n");
  uart1_write_unsafe("[CMD] STATUS              system + GPS + POST status\r\n");
  uart1_write_unsafe("[CMD] FAULTS              highest fault level\r\n");
  uart1_write_unsafe("[CMD] ECHO                connectivity check\r\n");
  uart1_write_unsafe("[CMD] CAPTURE             trigger payload camera\r\n");
  uart1_write_unsafe("[CMD] IMGDUMP             capture + hex dump JPEG on UART\r\n");
  uart1_write_unsafe("[CMD] ── Mode ──\r\n");
  uart1_write_unsafe("[CMD] MODE=<0-5|name>     BOOT|SAFE|DETUMBLE|NOMINAL|DIAGNOSTIC|PAYLOAD\r\n");
  uart1_write_unsafe("[CMD] DEPLOY              BOOT/SAFE → DETUMBLE (deploy panels)\r\n");
  uart1_write_unsafe("[CMD] DEPLOYCLEAR         reset deploy-in-progress flag\r\n");
  uart1_write_unsafe("[CMD] ── GPS ──\r\n");
  uart1_write_unsafe("[CMD] GPS                 last GPS fix\r\n");
  uart1_write_unsafe("[CMD] GPSSTATS            GPS statistics (rx/err/overflow)\r\n");
  uart1_write_unsafe("[CMD] RESETGPS            reset GPS stats\r\n");
  uart1_write_unsafe("[CMD] RESETGPS COLD       cold start GPS\r\n");
  uart1_write_unsafe("[CMD] ── Calibration ──\r\n");
  uart1_write_unsafe("[CMD] MAG-CAL-START       start magnetometer calibration\r\n");
  uart1_write_unsafe("[CMD] MAG-CAL-STOP        stop & compute offsets\r\n");
  uart1_write_unsafe("[CMD] MAG-CAL-STATUS      show magnetometer cal status\r\n");
  uart1_write_unsafe("[CMD] IMU-CAL-START       start IMU (gyro+accel) calibration\r\n");
  uart1_write_unsafe("[CMD] IMU-CAL-STOP        stop & compute offsets\r\n");
  uart1_write_unsafe("[CMD] IMU-CAL-STATUS      show IMU cal status\r\n");
  uart1_write_unsafe("[CMD] IMU-CAL-SAVE        save IMU cal to flash\r\n");
  uart1_write_unsafe("[CMD] IMU-CAL-LOAD        load IMU cal from flash\r\n");
  uart1_write_unsafe("[CMD] ── Tests ──\r\n");
  uart1_write_unsafe("[CMD] I2CSCAN             scan I2C bus\r\n");
  uart1_write_unsafe("[CMD] BH1750_TEST         test light sensor\r\n");
  uart1_write_unsafe("[CMD] SHT31_TEST          test temp/humidity sensor\r\n");
  uart1_write_unsafe("[CMD] RTC_TEST            test DS3231 RTC\r\n");
  uart1_write_unsafe("[CMD] POWER_TEST          test INA219 bus power\r\n");
  uart1_write_unsafe("[CMD] SOLAR_TEST          test solar panel ADC\r\n");
  uart1_write_unsafe("[CMD] ── Misc ──\r\n");
  uart1_write_unsafe("[CMD] LOG                 dump event log (TODO)\r\n");
  uart1_write_unsafe("[CMD] SETTIME YYYY MM DD HH MM SS   set RTC\r\n");
  uart1_write_unsafe("[CMD] HELP                this message\r\n");
  uart1_release_lock();
}
...
static void cmd_gps(const char *cmd)
{
  ...
  if (gps_get_last_fix(&fix))
  {
    ...
    const GpsStats_t *stats = gps_get_stats();
    snprintf(buf, sizeof(buf), "[CMD] GPS STATS: rx=%u chk_err=%u inv=%u valid=%u overflow=%u\r\n",
