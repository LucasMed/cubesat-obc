...
int csp_usart_open(const csp_usart_conf_t *conf,
  ...
  uart_set_format(driver_instance.uart_inst, conf->databits, conf->stopbits,
                  conf->paritysetting);
  ...
  // Create RX task
  if (xTaskCreate(uart_rx_task, "UART_RX", UART_RX_TASK_STACK_SIZE,
                  &driver_instance, UART_RX_TASK_PRIORITY,
                  &driver_instance.rx_task_handle) != pdPASS) {
