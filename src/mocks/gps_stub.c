...
static uint32_t g_call_counts[10] = {0}; // Index: 0-init, 1-deinit, 2-read_fix, 3-get_last_fix, 4-is_fix_valid, 5-get_sats, 6-set_data, 7-set_mode, 8-get_call_count, 9-reset_counters
static const char* function_names[10] = {"gps_init","gps_deinit","gps_read_fix","gps_get_last_fix","gps_is_fix_valid","gps_get_satellites_in_view","gps_mock_set_data","gps_mock_set_mode","gps_mock_get_call_count","gps_mock_reset_counters"};

// Internal function to match name to index
static int lookup_func_index(const char* name) {
    for(int i=0;i<10;i++) {
        if(strcmp(name,function_names[i])==0) return i;
    }
    return -1;
}
...
bool gps_is_fix_valid(void) {
    ...
    if(g_mock_mode==GPS_MOCK_FAULT_STALE_DATA || g_last_fix.timestamp_ms>=5000) return false;
...
uint32_t gps_mock_get_call_count(const char* func_name) {
    ...
    if(idx>=0) return g_call_counts[idx];
    return 0;
}
